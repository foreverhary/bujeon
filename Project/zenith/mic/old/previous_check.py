...
from mic.old.control.PreviousCheckControl import PreviousCheckControl
from mic.old.model.PreviousCheckModel import PreviousCheckModel
from mic.old.view.PreviousCheckView import PreviousCheckView
from process_package.component.CustomComponent import style_sheet_setting


class PreviousCheck(QApplication):
    def __init__(self, sys_argv):
        super(PreviousCheck, self).__init__(sys_argv)
        style_sheet_setting(self)
        self._model = PreviousCheckModel()
        self._control = PreviousCheckControl(self._model)
        self._view = PreviousCheckView(self._model, self._control)
        self._view.show()

        self._control.close_signal.connect(self._view.close)


if __name__ == '__main__':
    app = PreviousCheck(sys.argv)
    sys.exit(app.exec_())
